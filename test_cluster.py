...
def benchmark(dataset_size, dataset_dimension, n_clusters):
    huge_dataset_sizes = np.arange(1, 11) * dataset_size
    k_means = sklearn.cluster.KMeans(n_clusters=n_clusters)
    kmeans_float = cuml.KMeans(n_clusters=n_clusters)

    huge_cuml_data = benchmark_algorithm(huge_dataset_sizes,
                                         kmeans_float.fit, (), {},
                                         max_time=240, sample_size=2, dataset_dimension=dataset_dimension)
    huge_k_means_data = benchmark_algorithm(huge_dataset_sizes,
                                            k_means.fit, (), {},
                                            max_time=240, sample_size=2, dataset_dimension=dataset_dimension)
    results = pd.merge(huge_cuml_data, huge_k_means_data, on='x', how='left', suffixes=('_cuml', '_sklearn'))
